...
def list_folders(folder_url, start_date, end_date, level=0, results=None, filtered_results=None):
    ...
    try:
        ...
        # Display the folder name with full URL and the > symbol
        print(f"{indent}{Fore.RED}> {full_folder_url}{Fore.RESET}")

        for index, item in enumerate(folder_data.get('jobs', [])):
            job_url = item['url']
            print(f"{indent}  {Fore.CYAN} [{index + 1}] - {job_url}{Fore.RESET}")  # Subfolders and jobs printed with -
            ...
            print(f"{indent}  {Fore.WHITE}Total Minutes: {Style.BRIGHT} {total_minutes} minutes{Fore.RESET}")
...
def main():
    ...
    if start_date_str.strip() == "":
        ...
        try:
            ...
            print(f"{Fore.RED}Invalid start date format. Please use YYYY-MM-DD.{Fore.RESET}")
            return
    ...
    if end_date_str.strip() == "":
        ...
        try:
            ...
            print(f"{Fore.RED}Invalid end date format. Please use YYYY-MM-DD.{Fore.RESET}")
            return
    ...
    print(f"{Fore.GREEN}Report generation complete!{Fore.RESET}")
